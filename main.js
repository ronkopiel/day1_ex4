...
function createImgSection(array) {
    let imgContainer =`<div class="image-container">`;
    let infoContainer = `<div class="info-container">`;
    for (let i = 0; i < array.length; i++) {
        ...
        imgContainer += `<div class="image-block image-${i+1}"><img src=${imageElement}></div>`;
        infoContainer += `<div><h1>${hobbyElement}</h1><p>${infoElement}</p></div>`;

    }
    imgContainer += `</div>`
    infoContainer += `</div>`
    const object = document.getElementById("objects");
    object.innerHTML += imgContainer;
    object.innerHTML += infoContainer;
}  
